...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: CalendarProps) {
  return (
    <DayPicker
      ...
      className={cn(' useTw ', className)}
      classNames={{
        months: 'flex flex-col sm:flex-row space-y-4 sm:space-x-4 sm:space-y-0 justify-between',
        month: 'space-y-4',
        ...
        day: cn(
          ...
          'h-9 w-9 p-0 font-normal aria-selected:opacity-100 rounded-md text-base',
        ),
      ...
      components={{
        IconLeft: ({ ...props }) => <IconChevronLeft className="h-5 w-5 " />,
        IconRight: ({ ...props }) => <IconChevronRight className="h-5 w-5 " />,
      }}
      {...props}
    />
  )
